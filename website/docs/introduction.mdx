...
Zuord (from the German word for "assign" or "map") is a TypeScript-first toolbelt for deep object operations and fully synchronized type inferences.
...
- `zuord`: **Runtime functions** — `integrate`, `merge`, `evolve`, `pick`, `omit`, etc.
  ...
  ```ts
  const result = zuord.merge([
    { id: 1, name: 'Alice' },
    { id: 1, age: 30 },
    { name: 'Alice Smith', active: true } as const
  ]);

  // Value: { id: 1, name: 'Alice Smith', age: 30, active: true }
  // Type: { id: number; name: 'Alice Smith'; age: number; active: true }
  ```

- `Zuord`: **Compile-time types** — `Integrate`, `Merge`, `Evolve`, `Pick`, `Omit`, etc.

  ```ts
  type Result = Zuord.Merge<[
