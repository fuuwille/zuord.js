...
Zuord provides a schema-less, synchronized API for both runtime and compile-time operations. Every transformation exists in two aligned forms:
...
- `zuord`: **Runtime functions** — `integrate`, `merge`, `evolve`, `pick`, `omit`, etc.
  ...
  const result = zuord.merge([{ first: 'Kayra' }, { last: 'Özkaya' }, { active: true } as const]);

  // Value: { first: 'Kayra', last: 'Özkaya', active: true }
  // Type: { first: string; last: string; active: true }
  ```
...
- `Zuord`: **Compile-time types** — `Integrate`, `Merge`, `Evolve`, `Pick`, `Omit`, etc.
  ...
  type Result = Zuord.Merge<[{ first: string}, { last: string }, { active: true }]>;
